...
def plot_freq(infile, outfile=None, format=None, newfigure=True, marker='.', size=1, multiplot=False):
	"""
	...
	if type(infile) == str:
		...
		data = data.transpose()
		_doplot(data, outfile, format, newfigure, marker, size, infile)		

	elif type(infile) == list and flen == 1:
		data = np.loadtxt(infile[0])
		data = data.transpose()
		_doplot(data, outfile, format, newfigure, marker, size, infile[0])
		...
		for i, file in enumerate(infile):
			...
			if multiplot:
				...
				_domultiplot(data, i, j, format, marker, size, file)

			else:
				_doplot(data, outfile, format, False, marker, size, file)
...
def _doplot(data, outfile=None, format=None, newfigure=True, marker='.', size=1, label=None):
		plt.ion()
		if newfigure:
			fig, ax = plt.subplots()
		else:
...
def _domultiplot(data, x, y, format=None, marker='.', size=1, title=None, label=None):
	ax = plt.subplot(gs[x, y])
	ax.set_title(title)
	ax.set_xlabel('frequency (mHz)')
	ax.set_ylabel('Q')
	ax.set_xlim(1.855, 1.87)
	ax.set_ylim(321, 323)
